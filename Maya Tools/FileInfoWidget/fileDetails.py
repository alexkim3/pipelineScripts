### This code gets the information of an owner SID, which is used to display on File Info Widget
### source of code:
...
# sidToUserData is a dictionary containing informations about users and their SIDs. For security reason my file is not providable 
# however there are instructions how to generate a new SID dictionary in the bottom of the code
from sidToUserData import sidToUserName
...
class SID_NAME_USE(wintypes.DWORD):
    ...
    def __str__(self):
        if self.value not in self._sid_types:
            # print('self._sid_types', self._sid_types)
            # raise ValueError('invalid SID type')
...
def look_up_account_sid(sid, system_name=None):
    ...
    try:
        ...
        print('error in advapi32.LookupAccountSidW!')
...
def getFileOwner(filePath):
    sid = str(get_file_security(filePath).pOwner)
    if sid in sidToUserName:
        return sidToUserName[sid]
    else:
        return sid
...
def get_owner(filename):
    return sidToUserName[str(get_file_security(filename).pOwner)]


def getSidDict(fileLocation):
    sidToUserName = {str(get_file_security(filename).pOwner): os.path.splitext(os.path.basename(filename))[0] for filename in glob.glob(fileLocation)}
    return sidToUserName

# # If dictionary update is needed use this snippet to get the usernames from Linux. 
# # This code generates a .txt file from each user named as the user. Pass all *.txt as an argument to getSidDict()
# # which will create a dictionary connecting user name with SIDs.
#
# foreach u ( `getent passwd | tail -130 | awk 'FS=":" {print($1)}' | tail -129 | awk '{printf($1" ")}'` )
# touch $u.txt
# chmod 666 $u.txt
# sudo chown $u $u.txt
# end
